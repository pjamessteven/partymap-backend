...
event_artist_upvotes = db.Table('event_artist_upvotes',
                                ...
                                db.Column('artist_name', db.String, db.ForeignKey(
                                    'artists.name'), primary_key=True),
                                db.Column('event_date_id', db.Integer, db.ForeignKey(
                                    'event_dates.id'), primary_key=True)
                                )
...
class Artist(db.Model):
    ...
    created_at = db.Column(db.Integer, default=int(datetime.utcnow().strftime("%s"))*1000)
...
class EventArtist(db.Model):
    ...
    artist_name = db.Column(db.String(50), db.ForeignKey(
        'artists.name'), primary_key=True)
    event_date_id = db.Column(db.Integer, db.ForeignKey(
        'event_dates.id'), primary_key=True)
